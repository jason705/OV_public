...
firebase.database().ref('/vote_subject/'+key).once('value', function(snapshot) {
	var string='';
 	for(i=0;i<snapshot.val().choice.length;i++){
  	...
  	$('.vote_content').append("<h2>"+snapshot.val().votename+"</h2><form action='' name='voting'>"+string+"</form><p>已有"+snapshot.val().count+"人參與此投票</br>截止日期："+snapshot.val().endtime+"</br>");
});
